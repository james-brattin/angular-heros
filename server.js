...
app.get("/api/heros", function (req, res) {
  db.collection(HEROS_COLLECTION)
    .find({})
    .toArray(function (err, heros) {
      if (err) {
...
app.get("/api/heros/:id", function (req, res) {
  db.collection(HEROS_COLLECTION).findOne(
...
app.put("/api/heros/:id", function (req, res) {
  ...
  db.collection(HEROS_COLLECTION).updateOne(
    { _id: new ObjectID(req.params.id) },
    updateHero,
    function (err, hero) {
      if (err) {
...
app.delete("/api/heros/:id", function (req, res) {
  db.collection(HEROS_COLLECTION).deleteOne(
    { _id: new ObjectID(req.params.id) },
    function (err, result) {
      if (err) {
